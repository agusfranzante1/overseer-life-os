...
export function LoginForm() {
  ...
  const submit = async (e: React.FormEvent) => {
    e.preventDefault()
    if (!configured) {
      setError('Supabase no está configurado. Agregá NEXT_PUBLIC_SUPABASE_URL y NEXT_PUBLIC_SUPABASE_ANON_KEY a .env.local y reiniciá el dev server.')
      return
    }
    setLoading(true)
    setError(null)
    ...
    try {
      ...
      if (mode === 'signup') {
        ...
        if (e1) throw e1
        setInfo('Cuenta creada. Revisá tu mail para confirmar (si Email Confirmation está activado en Supabase).')
        setMode('login')
      } else {
    ...
    <div className="min-h-screen flex items-center justify-center bg-zinc-950 p-6">
      <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }}
        ...
        <div className="flex items-center gap-3 justify-center mb-8">
          ...
          <h1 className="text-2xl font-bold text-white tracking-wider uppercase">Overseer</h1>
        </div>
        ...
        <div className="bg-zinc-900 border border-zinc-800 rounded-2xl p-6 space-y-4">
          ...
          <div className="flex bg-zinc-950 border border-zinc-800 rounded-lg p-0.5">
            <button onClick={() => { setMode('login'); setError(null); setInfo(null) }}
              ...
              className={`flex-1 py-1.5 rounded-md text-xs font-bold transition-colors ${
                mode === 'login' ? 'bg-zinc-800 text-white' : 'text-zinc-500 hover:text-zinc-200'
              }`}>
            ...
            <button onClick={() => { setMode('signup'); setError(null); setInfo(null) }}
              type="button"
              className={`flex-1 py-1.5 rounded-md text-xs font-bold transition-colors ${
                mode === 'signup' ? 'bg-zinc-800 text-white' : 'text-zinc-500 hover:text-zinc-200'
              }`}>
          ...
          <form onSubmit={submit} className="space-y-3">
            {mode === 'signup' && (
              <div>
                <label className="text-[10px] font-mono uppercase tracking-wider text-zinc-500">Nombre (opcional)</label>
          ...
          {!configured && (
            <p className="text-[10px] text-zinc-500 leading-relaxed">
              Supabase no está configurado todavía. Pegá tu <code className="text-indigo-400">NEXT_PUBLIC_SUPABASE_URL</code> y
              <code className="text-indigo-400"> NEXT_PUBLIC_SUPABASE_ANON_KEY</code> en <code>.env.local</code> y reiniciá el dev server.
            </p>
          )}
        </div>
